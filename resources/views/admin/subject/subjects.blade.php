            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($subjects as $subject)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-left text-white">{{$subject['availSubject']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$subject['desc']}}</td>
                                
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button data-dropdown-toggle="dropdown-{{ $subject->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    ...
                                    <div id="dropdown-{{ $subject->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                            ...
                                            <li>
                                                <button type="button"
                                                    data-modal-target="editSubjectModal-{{ $subject->id }}"
                                                    data-modal-toggle="editSubjectModal-{{ $subject->id }}"
                                                    class="open-edit-modal block w-full text-left py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    Edit
                                                </button>
                                            </li>
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $subjects->firstItem() ?? 0 }}-{{ $subjects->lastItem() ?? 0 }}</span>
                        of
