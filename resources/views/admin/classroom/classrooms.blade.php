<x-admin.layout>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form action="{{ route('admin.classrooms.index') }}" method="GET" class="flex items-center">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($classrooms as $classroom)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-left text-white">{{$classroom['class']}}</td>
                                <td class="px-4 py-2 text-left text-white">
                                    @foreach ($classroom->students as $student)
                                        <span class="inline-block bg-gray-200 text-gray-800 rounded px-2 py-1 text-xs mr-1 mb-1">
                                            {{ $student->name }}
                                        </span>
                                    @endforeach
                                </td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button data-dropdown-toggle="dropdown-{{ $classroom->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                        </svg>
                                    </button>

                                    <div id="dropdown-{{ $classroom->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                            <li>
                                                <button type="button"
                                                    data-modal-target="showClassroomModal-{{ $classroom->id }}"
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $classrooms->firstItem() ?? 0 }}-{{ $classrooms->lastItem() ?? 0 }}</span>
                        of
                    ...
                    <li>
                        <li>
                            ...
                            @else
                                <a href="{{ $classrooms->previousPageUrl() }}"
                    ...
                    @foreach ($classrooms->links()->elements[0] as $page => $url)
                        <li>
            ...
            {{-- Modals Edit per classroom --}}
            @foreach ($classrooms as $classroom)
                 @include('admin.classroom.edit', ['classroom' => $classroom])
            @endforeach

            {{-- Modals SHOW per classroom --}}
            @foreach ($classrooms as $classroom)
                @include('admin.classroom.show', ['classroom' => $classroom])
            @endforeach
