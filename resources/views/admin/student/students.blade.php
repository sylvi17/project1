            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($students as $student)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-left text-white">{{$student['name']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$student->classroom->class}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$student['email']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$student['address']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$student['bday']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$student['gender']}}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button data-dropdown-toggle="dropdown-{{ $student->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    ...
                                    <div id="dropdown-{{ $student->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        ...
                                        <div class="py-1">
                                            <form action="{{ route('admin.students.destroy', $student->id) }}" method="POST" onsubmit="return confirm('Yakin ingin hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $students->firstItem() ?? 0 }}-{{ $students->lastItem() ?? 0 }}</span>
                        of
