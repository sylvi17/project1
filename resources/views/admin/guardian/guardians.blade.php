            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form action="{{ route('admin.guardians.index') }}" method="GET" class="flex items-center">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($guardians as $guardian)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-left text-white">{{$guardian['name']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$guardian['job']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$guardian['email']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$guardian['telpon']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$guardian['address']}}</td>
                                <td class="px-4 py-2 text-left text-white">{{$guardian['gender']}}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button data-dropdown-toggle="dropdown-{{ $guardian->id }}" class="inline-flex items-center p-0.5 text-sm font-medium text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                    ...
                                    <div id="dropdown-{{ $guardian->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                            ...
                                            <li>
                                                <button type="button"
                                                    data-modal-target="editGuardianModal-{{ $guardian->id }}"
                                                    data-modal-toggle="editGuardianModal-{{ $guardian->id }}"
                                                    class="open-edit-modal block w-full text-left py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    Edit
                                                </button>
                                            </li>
                                        ...
                                        <div class="py-1">
                                            <form action="{{ route('admin.guardians.destroy', $guardian->id) }}" method="POST" onsubmit="return confirm('Yakin ingin hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-full text-left py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Delete</button>
                                            </form>
                                        </div>
                                    </div>
                                </td>
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $guardians->firstItem() ?? 0 }}-{{ $guardians->lastItem() ?? 0 }}</span>
                        of
                ...
                <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add </h3>
